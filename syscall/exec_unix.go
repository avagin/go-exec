...
func StringSlicePtr(ss []string) []*byte {
	...
	for i := 0; i < len(ss); i++ {
		bb[i] = syscall.StringBytePtr(ss[i])
	}
	bb[len(ss)] = nil
	return bb
}
...
func CloseOnExec(fd int) { fcntl(fd, syscall.F_SETFD, syscall.FD_CLOEXEC) }
...
var execveLibc func(path uintptr, argv uintptr, envp uintptr) syscall.Errno
